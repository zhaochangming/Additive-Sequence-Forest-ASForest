        ...
        def _build_tree(X, y):
            ...
            def _split_traverse_node(node, container, node_temp, loss_temp,
                ...
                if verbose:
                    depth_spacing_str = " ".join([" "] * node["depth"])
                    print(
                        " {}node {} @ depth {}: loss={:.6f}, j_feature={}, threshold={:.3f}, N=({},{})".
                ...
                _, node["children"]["left"]["M5"], _ = _fit_model(
                    X_left, y_left, model, None, np.zeros(len(X_left)))
                ...
                _, node["children"]["right"]["M5"], _ = _fit_model(
                    X_right, y_right, model, None, np.zeros(len(X_right)))
                ...
                y_pred = np.array([
                    _predict_M5(root, x, self.categorical_feature_index)
                    for x in self.X
                ])
                self.M5Loss.append(self.loss(self.y, y_pred))
                # testing loss
                if self.test_X is not None:
                    ...
                    self.test_loss_M5.append(self.loss(self.test_y, y_pred))
                    y_pred = np.array([
                        _predict(root, x, 0, self.categorical_feature_index)
                        for x in self.test_X
                    ])
                    self.test_loss_PT.append(self.loss(self.test_y, y_pred))
                # decide split node
                split_index = loss_temp.index(max(loss_temp))
                _split_traverse_node(node_temp[split_index], container,
                                     node_temp, loss_temp, patch_num)
...
def _predict_M5(node, x, categorical_feature_index=None):
    if categorical_feature_index is not None:
        x_continus = x[categorical_feature_index]
    else:
        x_continus = x
    no_children = node["children"]["left"] is None and node["children"]["right"] is None
    if no_children:
        y_pred_x = node["M5"].predict(x_continus)[0]
        return y_pred_x
    else:
        if x[node["j_feature"]] <= node["threshold"]:  # x[j] < threshold
            return _predict_M5(node["children"]["left"], x,
                               categorical_feature_index)
        else:  # x[j] > threshold
            return _predict_M5(node["children"]["right"], x,
                               categorical_feature_index)
