...
class PatchTree(object):
    def __init__(self, max_patch=5, n_jobs=1, task='clf'):
        self.max_patch = max_patch
        self.tree = None
        self.train_Loss = []
        self.train_X = None
        self.train_y = None
        self.patch_num = 1  # patch_num>=2
        self.verbose = False
        self.n_classes = None
    ...
    def fit(self, train_X, train_y, test_X=None, test_y=None, verbose=False):
        ...
        if self.verbose:
            print(" max_patch={}, \n alpha_list={}, \n sample_leaf_list={}".format(
                self.max_patch, self.L2_list, sample_leaf_list))
        ...
        if not result["did_split"]:
            ...
            if self.verbose:
                ...
                print(" {}*leaf {} @ depth {}: loss={:.6f}, N={}".format(
                    ...
                    node["n_samples"]))
            #
            if self.max_patch is None:
                max_patch_condition = True
            else:
                max_patch_condition = (self.patch_num < self.max_patch)
            if max_patch_condition:
    ...
    def _splitter(self, node):
        ...
        if node["random_feature_index"] is not None:
            if self.task == 'clf':
                ...
                X_split = X[:, node["random_feature_index"]]
            # leaf samples
            min_samples_left, min_samples_right = 5, 5
            # split_options
            ...
            if self.max_patch is None:
                ...
                max_patch_condition = (self.patch_num < self.max_patch)
            if max_patch_condition and (N >= (min_samples_left + min_samples_right)):
                # feature_threshold
                feature_threshold = []
                ...
                if (best_loss_split is not None) and (best_loss_split < loss_best):
                    ...
                    alpha_temp = (alpha_left + alpha_right) / 2
