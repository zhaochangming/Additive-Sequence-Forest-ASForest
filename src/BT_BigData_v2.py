...
class BT(object):
    ...
    def _splitter(self, node):
        ...
        j_feature = None
        threshold = None
        garr, harr = self.calGrad(output, y), self.calHess(output, y)
        loss_best = self.calObj(garr, harr)
        #
        if node["random_feature_index"] is not None:
            ...
            if max_leafs_condition and (N >= (min_samples_left + min_samples_right)):
                ...
                for j_feature in range(X_split.shape[1]):
                    threshold_search = []
                    x_quantle = []
                    for i in range(1, _NUM_QUANTLE):
                        x_quantle.append(
                            np.quantile(
                                X_split[:, j_feature], i / _NUM_QUANTLE, interpolation="lower"))
                    threshold_search = list(set(x_quantle))
                ...
                # Split
                split_result_list = [
                    self._split_loss(f_t, split_options, garr, harr) for f_t in feature_threshold
                ]
                # Get split feature and threshold
                loss_list = [x for x in split_result_list]
                ...
                if (best_loss_split is not None) and (best_loss_split < loss_best):
                    ...
                    loss_left, model_left, data_left = self._fit_model(data_left)
                    loss_right, model_right, data_right = self._fit_model(data_right)
                    did_split = True
    ...
    def _split_loss(self, f_t, split_options, garr, harr):
        ...
        if not all(split_conditions):
            return None
        gl, gr = garr[idx_left], garr[idx_right]
        hl, hr = harr[idx_left], harr[idx_right]
        # Compute weight loss function
        loss_split = self.calObj(gl, hl) + self.calObj(gr, hr)
        return loss_split
